...
| --------------------------- | --------------------------------------------- | ------------ |
| `conus_run.sbatch`          | Full pipeline (`polygonize` + `postprocess`)  | ~30 min      |
| `prep_usda.sbatch`          | Convert + index the USDA CSB1825 reference    | ~10 min      |
...
| `eval_output.sbatch`        | Object-level + instance metrics on a run      | ~15 min      |
| `bench_eliminate.sbatch`    | Raster vs DuckDB eliminate benchmark          | ~10 min      |
| `tile_sweep.sbatch`         | Single-tile parameter sweep scored vs USDA    | ~20 min      |
