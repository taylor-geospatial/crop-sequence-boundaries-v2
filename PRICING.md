...
| --------------------------- | ---------------: | -------------: | ------------: | ---------------------------------------- |
| `csb polygonize 2018 2025`  |         22.2 min |             32 |        256 GB | 403 tile Parquet files, 15.44 M polygons |
| `csb postprocess 2018 2025` |          7.8 min |             32 |        256 GB | National and state GeoParquet files      |
...
dataset—take 30.2 minutes wall for the configuration of record
(`--reclass --usda-noise-filter 2 --roads-mask`). Parity preparation, parity evaluation, and PMTiles
...
| -------------------------- | ---------------: | -------------- | ---------: | --------------: |
| Polygonize and postprocess |           30 min | `m7i.16xlarge` | $0.9089/hr |           $0.45 |
...
| **Compute estimate**       | **about 52 min** | —              |          — | **about $1.05** |

The core 30-minute GeoParquet build accounts for about $0.45 of that estimate.
The full $1.05 includes the optional PMTiles and parity stages.
...
about **$1.17**. Keeping about 5 GB in S3 for a full year brings the modeled
run-and-retain total to roughly **$2.50**. These figures exclude taxes and
small EBS, public IPv4, and request charges. A complete retry would add about
$1.05 in compute; that is a contingency, not a worst-case bound.
...
| ------------------------------- | ----------------------: | -----------: |
| Wall time                       |                  120 hr |      0.50 hr |
| Reported/requested CPUs         |                      96 |           32 |
| Core-hours                      |                  11,520 |         16.1 |
