...
(:func:`csb.raster_eliminate.eliminate_label_raster`) against the DuckDB
polygon-side baseline (:mod:`csb.polygon_eliminate`) on the identical labeled
combine raster.
...
def _bench_impl(
    ...
    for _ in range(repeats):
        ...
        if p.exitcode != 0 and q.empty():
            runs.append(
                {"status": "crash", "error_type": "ProcessDied", "error": f"exitcode={p.exitcode}"}
            )
        else:
            runs.append(q.get())
...
def bench_tile(
    *,
    ...
    for size in sizes:
        ...
        combo, eff_per_combo, _cdl, transform = _combine_years_windowed(national_cdl, years, window)
