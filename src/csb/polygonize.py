    ...
    for i, year in enumerate(years):
        ...
        with rasterio.open(cdl_path) as src:
            ...
            if transform is None:
                ...
                if upsample > 1:
                    # Match USDA's 10m pipeline: same extent, finer grid.
                    transform = transform * rasterio.Affine.scale(1.0 / upsample)
        ...
        if use_reclass:
            ...
            if non_crop.any():
                key[non_crop] = barren
        # USDA's production filter runs on the RECLASSIFIED raster, i.e. on
        # the delineation key (per the USDA CSB team's shared preprocessing).
        if usda_noise_px >= 1:
        ...
        if upsample > 1:
            # Nearest-neighbour resample AFTER reclass+filter (ArcPy order:
            # split -> reclass -> filter -> resample). Categorical, so repeat.
            key = np.repeat(np.repeat(key, upsample, axis=0), upsample, axis=1)
            orig = np.repeat(np.repeat(orig, upsample, axis=0), upsample, axis=1)
...
def _phase1_polygonize(args: tuple[str, dict[str, Any]]) -> str:
    ...
    edges_mask_path: str | None = params.get("edges_mask")
    ...
    upsample: int = params.get("upsample", 1)
    pixel_size: float = 30.0 / upsample
    pixel_area: float = pixel_size * pixel_size
    ...
    if roads_mask_path:
        ...
        # Rasterize at the combine grid (which may be upsampled), not the
        # source 30m window dims, so thin roads burn at the finer resolution.
        roads = rasterize_roads_for_window(
            _Path(roads_mask_path), transform, mask.shape[1], mask.shape[0]
        )
        mask &= ~roads  # exclude road/rail pixels so fields split at roads
    if edges_mask_path:
        from pathlib import Path as _Path

        from csb.roads import edges_mask_for_window

        # USDA's exact 10m edge raster (value 1 = road/rail), warped to grid.
        edges = edges_mask_for_window(
            _Path(edges_mask_path), transform, mask.shape[1], mask.shape[0]
        )
        mask &= ~edges
    if not mask.any():
    ...
    if usda_retention:
        ...
        areas_lbl = label_areas(lbl, n_lbl, pixel_area)
        keep = (eff_lbl >= min_cropland) | ((areas_lbl >= min_area_keep) & (eff_lbl >= 1))
        keep[0] = False
    ...
    lbl, n_lbl = eliminate_label_raster(lbl, n_lbl, thresholds, pixel_area, pixel_size)
    if n_lbl == 0:
    ...
    areas_per_label = label_areas(lbl, n_lbl, pixel_area)
...
def run_polygonize(
    *,
    ...
    p1_params = {
        ...
        "edges_mask": str(edges_mask) if edges_mask else None,
