...
def edges_mask_for_window(
    ...
    *,
    ...
    """Boolean HxW road/rail mask from USDA's Edges_10M GeoTIFF.

    USDA's exact edge raster (value 1 = road/rail) is 10 m in EPSG:4326. A
    ``WarpedVRT`` reprojects and resamples it onto this tile's grid
    (``transform``/``width``/``height``, in ``dst_crs``). Downsampling uses
    ``Resampling.max`` so a 30 m cell counts as edge if *any* underlying 10 m
    edge pixel falls in it — thin roads are preserved, fields split cleanly.
    Some builds ship the CRS only in the ``.prj``/``.tfw``; assume 4326 when
    the raster has no embedded CRS.
    """
    with rasterio.open(edges_tif) as src:
