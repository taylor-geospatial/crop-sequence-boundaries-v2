...
- **~30 minutes** for the 8-year CONUS polygonize and postprocess stages on a
...
- **87.5% coverage IoU vs USDA ground truth over the full CONUS**
    (15.44M polygons) with the configuration of record:
    `--reclass --usda-noise-filter 2 --roads-mask <tiger-roads.parquet>`.
...
# 3. Build a road/rail mask (one-time). TIGER lines match USDA's method:
csb roads-prep -o data/tiger_roads.parquet
#    (or Overture instead: csb roads-prep --source overture -o data/roads.parquet)

# 4. Run the full pipeline with the configuration of record.
...
| `roads-prep`       | Download road/rail centerlines into an indexed `--roads-mask` GeoParquet — Census TIGER (default, USDA's method) or Overture.     |
...
Extra commands handle validation and serving:

| Stage         | What it does                                                                              |
| ------------- | ----------------------------------------------------------------------------------------- |
| `parity-prep` | Hilbert-sort + add bbox columns to ours and USDA parquets so DuckDB can prune row groups. |
| `parity`      | 16-region (or `--whole-conus`) IoU/acreage validation vs USDA ground truth.               |
| `pmtiles`     | Build a CONUS PMTiles archive from the national parquet (requires `tippecanoe` on PATH).  |

`csb --help` lists the rest: `visualize` (4-panel comparison PNGs),
`object-eval` / `instance-metrics` (per-polygon agreement metrics),
`bench-eliminate`, and `tile-sweep`.

## Configuration

Pipeline settings are CLI options with defaults in `src/csb/config.py`. Use
`--help` on a command to see the complete set.

The **configuration of record** — the one validated against USDA ground truth —
adds USDA's CDL preprocessing on top of the defaults:
`--reclass` (USDA's temp-general-code reclass table, bundled at
`src/csb/data/CDL_tempGeneralCode.csv`), `--usda-noise-filter 2` (USDA's
production ≤2-pixel speckle filter), and `--roads-mask` with a TIGER-derived
roads GeoParquet. All three stages were provided by the USDA CSB team and are
described in the accompanying paper. Other tuning example:
...
With the configuration of record
(`--reclass --usda-noise-filter 2 --roads-mask <tiger-roads.parquet>`), the
8-year CONUS run produces **15.44M polygons** with **87.5% coverage IoU**
...
map to **~$1.05 of compute**. One month of S3 storage brings the model to
**~$1.17**. Assumptions and the public-list ArcGIS comparison are in
[PRICING.md](PRICING.md).
